...
const statusConfig: Record<string, { label: string; dotColor: string; textColor: string }> = {
  ...
  active: { label: "Active", dotColor: "bg-green-500", textColor: "text-green-600" },
}
...
export function LeadDetailPanel({ lead, onClose, onSendMessage }: LeadDetailPanelProps) {
  ...
  const statusInfo = statusConfig[status] || { label: status, dotColor: "bg-slate-400", textColor: "text-slate-500" }
  const sourceInfo = source === "whatsapp" 
    ? { label: "WhatsApp", color: "bg-emerald-50 text-emerald-700" }
    : { label: "Email", color: "bg-blue-50 text-blue-700" }
      ...
      <div className="flex items-center justify-between border-b border-slate-200 p-4 bg-gradient-to-r from-white to-blue-50/30">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className={cn("text-xs font-medium", statusInfo.textColor)}>{statusInfo.label}</span>
          </div>
        </div>
        <Button 
          variant="ghost" 
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="mt-6 flex items-center gap-3 rounded-lg border border-indigo-200 bg-indigo-50/50 p-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm text-indigo-600">{lead.session?.aiRecommendation || lead.ratingReason || "No recommendation yet"}</p>
          </div>
        </div>

        <div className="mt-6">
          <h4 className="text-sm font-semibold text-slate-700 mb-3">AI Insights</h4>
          <div className="grid grid-cols-3 gap-3">
            <div className={cn(
              "rounded-xl p-4 text-center",
              rating >= 4 ? "bg-green-50 border border-green-200" :
              rating >= 3 ? "bg-blue-50 border border-blue-200" :
              "bg-slate-50 border border-slate-200"
            )}>
              <p className="text-xs text-slate-500 mb-1">Conversion</p>
              <p className={cn(
                "text-lg font-bold",
                rating >= 4 ? "text-green-600" : rating >= 3 ? "text-blue-600" : "text-slate-500"
              )}>
                {rating >= 4 ? "High" : rating >= 3 ? "Medium" : "Low"}
              </p>
              <p className="text-xs text-slate-400 mt-1">
                {rating >= 4 ? "85% chance" : rating >= 3 ? "50% chance" : "20% chance"}
              </p>
            </div>
            <div className="rounded-xl p-4 text-center bg-purple-50 border border-purple-200">
              ...
              <p className="text-lg font-bold text-purple-600">
                {rating >= 4 ? "$5k+" : rating >= 3 ? "$1k-5k" : "<$1k"}
              </p>
              <p className="text-xs text-slate-400 mt-1">
                {rating >= 4 ? "Enterprise" : rating >= 3 ? "SMB" : "Starter"}
              </p>
            </div>
            <div className={cn(
              "rounded-xl p-4 text-center",
              rating >= 4 ? "bg-red-50 border border-red-200" :
              rating >= 3 ? "bg-amber-50 border border-amber-200" :
              "bg-slate-50 border border-slate-200"
            )}>
              <p className="text-xs text-slate-500 mb-1">Urgency</p>
              <p className={cn(
                "text-lg font-bold",
                rating >= 4 ? "text-red-600" : rating >= 3 ? "text-amber-600" : "text-slate-500"
              )}>
                {rating >= 4 ? "Hot" : rating >= 3 ? "Warm" : "Cold"}
              </p>
              <p className="text-xs text-slate-400 mt-1">
                {rating >= 4 ? "Act now" : rating >= 3 ? "This week" : "When ready"}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 rounded-lg border border-indigo-200 bg-indigo-50/50 p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h4 className="font-semibold text-indigo-700">AI Recommendation</h4>
          </div>
          <div className="space-y-2">
            <div className="flex items-start gap-3">
              <div className={cn(
                "flex h-8 w-8 items-center justify-center rounded-full shrink-0",
                rating >= 4 ? "bg-indigo-100" : rating >= 3 ? "bg-blue-100" : "bg-slate-100"
              )}>
                <Target className={cn(
                  "h-4 w-4",
                  rating >= 4 ? "text-indigo-600" : rating >= 3 ? "text-blue-600" : "text-slate-500"
                )} />
              </div>
              <div>
                <p className="text-sm font-medium text-slate-700">Priority Level</p>
                <p className="text-xs text-slate-500 mt-0.5">
                  {rating >= 4 ? "High priority - Immediate attention recommended" 
                    : rating >= 3 ? "Medium priority - Follow up within 48 hours"
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  {rating >= 4 ? "High conversion likelihood - Strong interest signals" 
                    : rating >= 3 ? "Moderate conversion potential - Standard follow-up"
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  {rating >= 4 ? "Contact within 24 hours for best results" 
                    : status === "manual" ? "Review and categorize before proceeding"
                    : rating >= 3 ? "Schedule follow-up call within 48 hours"
